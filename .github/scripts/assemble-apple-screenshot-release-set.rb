...
class AppleScreenshotReleaseSetAssembler
  ...
  def reject_hidden_approval_value!(value, label)
    ...
    when String
      if value.match?(/approved-for-build8-upload|signed.?release.?parity.?approved/i)
        raise AppleScreenshotReleaseSetAssemblyError,
  ...
  def reject_approval_material!(candidate)
    ...
    if serialized.include?("approved-for-build8-upload") || serialized.include?("signedReleaseParityApproved")
      raise AppleScreenshotReleaseSetAssemblyError,

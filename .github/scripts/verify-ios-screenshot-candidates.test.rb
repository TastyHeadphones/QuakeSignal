...
class IOSBuild8ScreenshotCandidateValidatorTest < Minitest::Test
  ...
  def candidate_manifest
    {
      ...
      "rootDirectory" => "screenshots-v1.1-build15",
      "captureEvidence" => {
  ...
  def candidate_provenance
    files = IOSBuild8ScreenshotCandidateValidator::EXPECTED_RELATIVE_PATHS.map do |relative_path|
      screenshot = @store.join("screenshots-v1.1-build15", relative_path)
  ...
  def test_symlinked_screenshot_root_locale_class_and_file_are_rejected
    cases = {
      "root" => lambda do |target|
        source = @store.join("screenshots-v1.1-build15")
        FileUtils.mv(source, target)
        File.symlink(target, source)
      end,
      "locale" => lambda do |target|
        source = @store.join("screenshots-v1.1-build15", "en-US")
        FileUtils.mv(source, target)
        File.symlink(target, source)
      end,
      "class" => lambda do |target|
        source = @store.join("screenshots-v1.1-build15", "en-US", "iphone-6.5")
        FileUtils.mv(source, target)
        File.symlink(target, source)
      end,
      "file" => lambda do |target|
        source = @store.join(
          "screenshots-v1.1-build15", "en-US", "iphone-6.5", "01-home.jpg"
        )
        FileUtils.mv(source, target)
        File.symlink(target, source)
      end,
    ...
    cases.each do |name, mutate|
      FileUtils.rm_rf(@store.join("screenshots-v1.1-build15"))
      remove_candidate_metadata
      build_screenshot_tree
      write_candidate
  ...
  def test_rejects_extra_or_missing_actual_files
    build_screenshot_tree
    extra = @store.join("screenshots-v1.1-build15", "en-US", "iphone-6.5", "extra.jpg")
    extra.write("extra")
    assert_rejected
    remove_candidate_metadata
    ...
    missing = @store.join("screenshots-v1.1-build15", relative_path)

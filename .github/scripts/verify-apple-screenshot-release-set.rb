...
class AppleScreenshotReleaseSetValidator
  INDEX_PATH = "ios/AppStore/screenshot-set-index-v1.1-build11.json"
  RELEASE_ROOT = "ios/AppStore/screenshot-release-sets-v1.1-build11"
  ...
  PLAN_PATHS = {
    "ios-ipados" => "ios/AppStore/screenshot-manifest-v1.1-build11.template.json",
    "tvos" => "ios/AppStore/platforms/tvos/screenshot-manifest-v1.1-build11.json",
    "watchos" => "ios/AppStore/platforms/watchos/screenshot-manifest-v1.1-build11.json",
    "visionos" => "ios/AppStore/platforms/visionos/screenshot-manifest-v1.1-build11.json",
    "maccatalyst" => "ios/AppStore/platforms/maccatalyst/screenshot-manifest-v1.1-build11.json",
  }.freeze
  HISTORICAL_EVIDENCE = [
    {
  ...
  def validate!(require_release_ready: false, expected_source_commit: nil)
    ...
    unless index_path.exist? || index_path.symlink?
      if require_release_ready
        raise AppleScreenshotReleaseSetValidationError,
              "a complete active build-11 screenshot release set is required"
      end
      return :pending
    end
  ...
  def validate_release_approval!(
    ...
    require_equal!(approval.fetch("status"), "approved-for-build11-upload", "release approval status")
...
if $PROGRAM_NAME == __FILE__
  begin
    ...
    if result == :pending
      puts "Historical Apple screenshot evidence is locked; final 26-frame build-11 release set is pending."

...
| Production training-test claim: opaque App Attest key identifier, UTC claim/expiry timestamps, and a same-day claim count after an immediate or reviewed delayed production test | Enforcing at most ten clearly labelled production training attempts per App Attest key per UTC day; this contains no APNs token, proof, or request body |
...
cleanup failure can delay deletion. The claim enforces ten production training
attempts per key per UTC day. Immediately before either production training

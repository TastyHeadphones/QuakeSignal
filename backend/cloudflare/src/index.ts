...
const DELIVERY_DEDUP_RETENTION_MS = 15 * 24 * 60 * 60_000;
...
// day has ended.
const PRODUCTION_TRAINING_TEST_PUSH_CLAIM_RETENTION_MS = 15 * 24 * 60 * 60_000;
...
export async function deactivateBadDeviceToken(
  ...
  try {
    ...
    const results = await db.batch([
      ...
      // evidence. Preserve each such revision as its own 15-day fence first.
      db
        .prepare(
          `INSERT OR IGNORE INTO apns_registration_revision_fences (
             token_hash, registration_revision, app_attest_key_id,
...
export async function completeAttestedRegistration(
  ...
  const priorTokenCleanup = [
    ...
    // 15-day cleanup. A pre-send intent that crashed after its D1 acceptance
    ...
    return legalPage(
      ...
      [
        ...
        },
        {
          ...
          body: "The separate Windows desktop app, legacy Tauri macOS builds (dormant for Apple release 1.1 build 15), and Chrome extension connect directly to Wolfx and keep preferences and recent event state locally. They do not register with the QuakeSignal iPhone/iPad notification service. Check the app or extension connection status, selected sources and thresholds, local notification permission, and network access when troubleshooting.",
        },
        {

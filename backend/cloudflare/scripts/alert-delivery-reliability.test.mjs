...
test("shipped normalize-to-notify path maps Wolfx EEW and USGS/EMSC catalogs", async () => {
  ...
  const wolfxEvent = normalizeJmaEew({
  ...
  assert.equal(usgsPayload.sourceId, "usgs_eqlist");
  assert.equal(usgsPayload.aps.alert["title-loc-args"][0], "USGS");
  assert.ok(usgsPayload.aps.alert);

  const emscEvents = normalizeCatalogGeoJSON("emsc_eqlist", {
    type: "FeatureCollection",
    features: [
      {
        type: "Feature",

...
RELEASE_WORKFLOW = "QuakeSignal 1.1 (13) Native Release"
...
APP_ATTEST_FINGERPRINT = "sha256:ejpiqRSvXEi0f9-V-gplyDfFSA7N2oP0KptOFe-n9Jc"
XCODE_SOURCE_GRAPH_FINGERPRINT = "sha256:2f8ec-nWPKKhs3BcjqNBnkWnkh5XtPwlWXd2MTSHfXQ"
...
LEGAL_PAGE_CONTRACTS = (
    {
        ...
        "requiredText": (
            ...
            "separate Windows desktop app, legacy Tauri macOS builds (dormant for Apple release 1.1 build 13), and Chrome extension",
...
def calculate_app_attest_fingerprint(repository_root: Path) -> str:
    ...
    if allowed != sorted(str(value) for value in range(1, 14)):
        fail("APP_ATTEST_ALLOWED_BUNDLE_VERSIONS must be exactly 1 through 13.")
    try:
...
def verify_bounded_source_contract(repository_root: Path) -> None:
    ...
    if len(re.findall(r"CURRENT_PROJECT_VERSION = 13;", generated)) != 3:
        fail("generated Xcode project must contain exactly three build-13 settings.")
...
def verify_live_worker_release(
    *, fetcher: Callable[..., Tuple[int, Dict[str, str], bytes]] = _request
    ...
    if (
        ...
        fail("live App Attest fingerprint/allow-list does not match release build 13.")

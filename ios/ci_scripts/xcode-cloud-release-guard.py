...
RELEASE_WORKFLOW = "QuakeSignal 1.1 (14) Native Release"
...
APP_ATTEST_FINGERPRINT = "sha256:bJmdD2ehT8pnnIdeeD_sBWa3hX4gb9NPdrV9AUAU1LE"
XCODE_SOURCE_GRAPH_FINGERPRINT = "sha256:Xy5yY-pH53K95P2uGf8Tv2dVs67cHKzyHRHIQJl6UFk"
...
LEGAL_PAGE_CONTRACTS = (
    {
        ...
        "requiredText": (
            ...
            "separate Windows desktop app, legacy Tauri macOS builds (dormant for Apple release 1.1 build 14), and Chrome extension",
...
def calculate_app_attest_fingerprint(repository_root: Path) -> str:
    ...
    if allowed != sorted(str(value) for value in range(1, 15)):
        fail("APP_ATTEST_ALLOWED_BUNDLE_VERSIONS must be exactly 1 through 14.")
    try:
...
def verify_bounded_source_contract(repository_root: Path) -> None:
    ...
    if len(re.findall(r"CURRENT_PROJECT_VERSION = 14;", generated)) != 3:
        fail("generated Xcode project must contain exactly three build-14 settings.")
...
def verify_live_worker_release(
    *, fetcher: Callable[..., Tuple[int, Dict[str, str], bytes]] = _request
    ...
    if (
        ...
        fail("live App Attest fingerprint/allow-list does not match release build 14.")

...
- [ ] Identify the exact signed public `Release` 1.1 archive/IPA, source
  ...
  `app-store-connect-portal-audit-2026-08-24.md`; they do not prove build 16.
- [ ] Confirm that build `1.1 (16)` is not the legacy TestFlight `1.0 (2)`
  ...
  background-training control; retain the build-16 release-contract result.
...
- [ ] Complete Mac Catalyst QA, exact approved screenshots, signed-Release
  ...
  TestFlight processing are historical only; build 16 must complete its own
...
- [ ] Capture a complete five-platform Debug Simulator candidate for the
  build-16 source, then retain it as unapproved until the protected finalizer
...
- [ ] Complete Apple Watch/tvOS/visionOS/Mac screenshot approval, remaining
  privacy/App Motion answers, and platform QA before submission. Build 16
...
- [ ] Record build 16's exact signed/deployed source inventory as `jma_eew` and `jma_eqlist`,

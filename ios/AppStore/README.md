...
| Current build-15 Apple release-set pointer | `screenshot-set-index-v1.1-build15.json` and `screenshot-release-sets-v1.1-build15/<source-commit>/` |
...
own source commit; they cannot be attached to the current source. Build 15 is
...
remain in the 2026-08-24 audit as historical evidence. Build 15 must receive
...
> evidence. Do not relabel or upload it for build 15. The separate build-8
...
> and explicitly unapproved. The build-15 candidate must be captured afresh and
...
is retained as historical evidence. The build-15
[`screenshot-set-index-v1.1-build15.json`](./screenshot-set-index-v1.1-build15.json)
...
Apple Vision Pro, and 5 Mac Catalyst images. A build-15 release set belongs
only under `screenshot-release-sets-v1.1-build15/<40-character-source-commit>/`;
...
- No build-15 Japanese or Simplified Chinese screenshot set is captured or
...
For build 15, release operators use only the two canonical hosted workflows in
...
`screenshot-set-index-v1.1-build15.json` remains pending in Git. Only the
...
6. Build and upload `1.1 (15)` through the protected GitHub workflows described in
...
7. Complete age rating, content rights, privacy, export compliance, localized
   ...
   third-party rights.
8. After protected upload and processing, test release candidate `1.1 (15)` on
   physical hardware
...
10. Only after build `1.1 (15)` is uploaded, processed, physically verified,

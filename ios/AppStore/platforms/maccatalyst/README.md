...
Retina display. Capture the window's native `2560 × 1600` pixels; never resize
the result. The Debug-only
fixture path requires both:
...
From an exact clean source commit, capture the complete set locally with:

```bash
ios/ScreenshotAutomation/capture-maccatalyst-screenshot-set.sh \
  /Volumes/RC20/quakesignal-maccatalyst-build8
```

Or capture one selector into its own atomic evidence directory:

```bash
ios/ScreenshotAutomation/capture-maccatalyst-screenshot.sh \
  maccatalyst-map \
  /Volumes/RC20/quakesignal-maccatalyst-map
```
...
evidence for the launched PID, then requires identical Core Graphics PID,
window ID, and 1280 × 800 logical-frame observations immediately before and
after native capture. Geometry readiness is followed by a fixed 10-second
...
access. The same native ScreenCaptureKit helper performs the one-time Screen
Recording preflight, binds the launched PID and Core Graphics window ID, asks
for best-resolution 2560 × 1600 output with `scalesToFit=false`, and records
that no post-capture resize occurred. Screen Recording access is mandatory.

Raw alpha-bearing window bytes are retained and hashed. A Core
Graphics/ImageIO transform composites those same 2560 × 1600 pixels over
`[0, 0, 0, 255]`, emits opaque PNG without resize, and records both hashes.
...
backing scale, host/Xcode versions, per-frame logs, and aggregate provenance.
Every reviewer and approval field remains `null`; capture never constitutes
upload approval. The harness is intentionally local and is not part of GitHub
Actions or Xcode Cloud.
...
`2880 × 1800` pixels. This plan selects native `2560 × 1600` Retina captures
consistently for all five English (U.S.) frames. The local capture harness
losslessly composites the native window alpha over fixed black without

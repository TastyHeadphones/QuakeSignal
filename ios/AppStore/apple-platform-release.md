# Coordinated Apple platform release 1.1 (13)
...
this runbook for build 13 unless an initial Xcode Cloud workflow is created by an
authorized release owner outside this run.

Configure exactly one Xcode Cloud workflow named
`QuakeSignal 1.1 (13) Native Release`. A single workflow is required because
...
For the final build-13 workflow, set **Deployment Preparation** to
...
Before starting build 13, retain a portal screenshot or App Store Connect API
...
origin to become fully ready and run its build-13 remote smoke contract. Mac
...
policy lock. Do not dispatch these commands until the Worker build-13 policy and
...
source, Worker build-13 policy, distribution profiles/certificates/API-key
...
- Capture and hash screenshots from the frozen build-13 binaries at Apple’s
  accepted sizes. The Mac source-only plan is
[`platforms/maccatalyst/screenshot-manifest-v1.1-build13.json`](./platforms/maccatalyst/screenshot-manifest-v1.1-build13.json):
...
`1.1` only after the corresponding build-13 release evidence is frozen. Do not

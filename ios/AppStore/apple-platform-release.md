# Coordinated Apple platform release 1.1 (15)
...
this runbook for build 15 unless an initial Xcode Cloud workflow is created by an
authorized release owner outside this run.

Configure exactly one Xcode Cloud workflow named
`QuakeSignal 1.1 (15) Native Release`. A single workflow is required because
...
For the final build-15 workflow, set **Deployment Preparation** to
...
Before starting build 15, retain a portal screenshot or App Store Connect API
...
archive, team `5TT564H883`, build `11`, the exact shared workflow name, and the
...
origin to become fully ready and run its build-15 remote smoke contract. Mac
...
policy lock. Do not dispatch these commands until the Worker build-15 policy and
...
source, Worker build-15 policy, distribution profiles/certificates/API-key
...
- Capture and hash screenshots from the frozen build-15 binaries at Apple’s
  accepted sizes. The Mac source-only plan is
[`platforms/maccatalyst/screenshot-manifest-v1.1-build15.json`](./platforms/maccatalyst/screenshot-manifest-v1.1-build15.json):
...
`1.1` only after the corresponding build-15 release evidence is frozen. Do not

# Coordinated Apple platform release 1.1 (14)
...
this runbook for build 14 unless an initial Xcode Cloud workflow is created by an
authorized release owner outside this run.

Configure exactly one Xcode Cloud workflow named
`QuakeSignal 1.1 (14) Native Release`. A single workflow is required because
...
For the final build-14 workflow, set **Deployment Preparation** to
...
Before starting build 14, retain a portal screenshot or App Store Connect API
...
origin to become fully ready and run its build-14 remote smoke contract. Mac
...
policy lock. Do not dispatch these commands until the Worker build-14 policy and
...
source, Worker build-14 policy, distribution profiles/certificates/API-key
...
- Capture and hash screenshots from the frozen build-14 binaries at Apple’s
  accepted sizes. The Mac source-only plan is
[`platforms/maccatalyst/screenshot-manifest-v1.1-build14.json`](./platforms/maccatalyst/screenshot-manifest-v1.1-build14.json):
...
`1.1` only after the corresponding build-14 release evidence is frozen. Do not

...
| Release candidate | Version `1.0 (6)` is already Ready for Distribution. Builds `1.1 (7)` through `1.1 (16)` are historical TestFlight or superseded evidence. The coordinated native candidate is `1.1 (17)`: its source-bound protected uploads completed, while App Store Connect processing confirmation, platform/physical-device QA, approved screenshots, public attachment, and App Review remain **PENDING**. Builds `1.0 (2)` through `1.0 (5)` remain historical QA or superseded. | App Store Connect record `6800642443`; `app-store-connect-build17-handoff-2026-08-24.md`; `README.md` build-number rule |

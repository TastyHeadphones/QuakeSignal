...
| tvOS | JMA-only foreground large-screen dashboard; reads `jma_eew` and `jma_eqlist` while open, with no APNs, App Attest, automatic alert audio, or background emergency alerts. System is visual-only; the Urgent and Japanese Voice sounds play only after an explicit Siri Remote action | The exact `1.1` description and review notes are saved. The three retained `1920 × 1080` Debug Simulator images are historical and non-uploadable; no build or screenshots are selected, and Apple TV Privacy Policy remains blank. **PENDING** final recapture, build 8, profile, platform QA, reviewed privacy text, signed parity, and named screenshot review |
| visionOS | JMA-only full native windowed foreground app. Apple lists App Attest for visionOS, but not Push Notifications or Time Sensitive Notifications. QuakeSignal's protected alert registration requires App Attest plus APNs, so this target does not start that registration path when APNs is unavailable and has no Time Sensitive, Critical Alerts, or background emergency-delivery path. A qualifying JMA-issued update may appear in-app and play the selected local sound only while the app is open; QuakeSignal does not calculate a new local-intensity or arrival-time forecast. | The 2026-08-22 audit saved the then-current `1.1` description and review notes. Their corrected App Attest/APNs wording must be resaved during the final authenticated pass. The five retained Debug Simulator images are historical and non-uploadable; no build or screenshots are selected, and App Motion remains **Set Up**. **PENDING** corrected copy save, final recapture, build 8, profile, QA, final App Motion answer, signed parity, and named review |
| Mac Catalyst | JMA-only SwiftUI native Mac storefront build from the shared `QuakeSignal` target; foreground/local reports, map, guide, settings, and full-window fresh-warning presentation with the selected System, Urgent, or Japanese Voice sound. It has no independent APNs, App Attest, background-alert, or Critical Alert path, and stops monitoring/audio when inactive. | **SELECTED** as the sole Mac route. The shared draft is saved as `1.1` with exact metadata, manual release, and Designed for iPad on Mac disabled; Tauri record `6800642853` remains unused. **PENDING** protected GitHub signed build 8, Mac QA, exact screenshots, signed parity, and named approval |
...
`app-store-connect-portal-audit-2026-08-22.md` for the current saved state and

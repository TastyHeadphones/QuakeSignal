...
fixture selector, waits for app-written stable 1280x800@2x geometry evidence,
captures that exact PID/window ID at native 2560x1600 pixels, and losslessly
alpha-composites over fixed black without resizing. The output is an atomic,
explicitly unapproved evidence directory outside the repository.
...
manifest_file="ios/AppStore/platforms/maccatalyst/screenshot-manifest-v1.1-build8.json"
...
if [ "$planned_selector" != "$frame_selector" ] || \
  ...
  echo "error: selector disagrees with the native Retina capture contract" >&2
  exit 65
fi
...
capture_helper="$temporary_root/maccatalyst-capture-window"
...
quakesignal_maccatalyst_run_tracked \
  xcrun swiftc -O "$script_dir/maccatalyst-capture-window.swift" -o "$capture_helper"
...
if [ "$bundle_identifier" != "com.quakesignal.app" ] || \
  ...
  echo "error: built app identity/version differs from the build-8 Catalyst contract" >&2
  exit 65
fi
...
while [ "$capture_attempt_count" -lt 2 ]; do
  ...
  if ! /usr/bin/ruby -rjson -rtime -e '
    ...
    expected_keys = %w[backingScale captureSelector logicalFrame processId reason recordedAtUtc schemaVersion status]
    ...
    abort "geometry scale differs" unless record["backingScale"] == 2
  ...
  if ! quakesignal_maccatalyst_run_tracked \
    "$capture_helper" "$app_pid" "$window_id" 1280 800 \
      "$attempt_raw" "$attempt_native_capture"; then
    echo "error: native ScreenCaptureKit window capture failed" >&2
    exit 70
  fi
  ...
  ' "$attempt_window_before" "$attempt_window_after"; then
    echo "error: Catalyst PID/window/frame changed across native capture" >&2
    exit 70
  fi
  if ! /usr/bin/ruby -rjson -e '
    capture = JSON.parse(File.read(ARGV.fetch(0)))
    window = JSON.parse(File.read(ARGV.fetch(1)))
    expected_keys = %w[captureApi captureResolution ignoreShadowsSingleWindow logicalFrame pixels postCaptureResizePerformed processId scalesToFit schemaVersion showsCursor windowId]
    abort "native-capture evidence keys differ" unless capture.keys.sort == expected_keys.sort
    abort "native-capture method differs" unless capture["schemaVersion"] == 1 && capture["captureApi"] == "ScreenCaptureKit.SCScreenshotManager"
    abort "native-capture PID/window differs" unless capture["processId"] == window["processId"] && capture["windowId"] == window["windowId"]
    abort "native-capture logical frame differs" unless capture["logicalFrame"] == window["logicalFrame"]
    abort "native-capture pixels differ" unless capture["pixels"] == [2560, 1600]
    abort "native-capture configuration differs" unless capture["captureResolution"] == "best" && capture["scalesToFit"] == false && capture["postCaptureResizePerformed"] == false && capture["showsCursor"] == false && capture["ignoreShadowsSingleWindow"] == true
  ' "$attempt_native_capture" "$attempt_window_before"; then
    echo "error: native ScreenCaptureKit evidence is invalid" >&2
    exit 70
  fi
  ...
  if [ "$raw_width" != "$expected_width" ] || [ "$raw_height" != "$expected_height" ]; then
    echo "error: native window capture is ${raw_width}x${raw_height}; expected 2560x1600" >&2
  ...
  if [ "$final_width" != "$expected_width" ] || [ "$final_height" != "$expected_height" ] || [ "$final_alpha_value" != "no" ]; then
    echo "error: final PNG must remain opaque native 2560x1600" >&2
    exit 65
  fi
  if ! /usr/bin/ruby -rjson -e '
    record = JSON.parse(File.read(ARGV.fetch(0)))
...
/usr/bin/ruby -rjson -rtime -e '
  ...
  if semantic_retry_performed
    rejection = JSON.parse(File.read(ARGV.fetch(5)))
    ...
    abort "unexpected semantic rejection argument" unless ARGV.fetch(5) == "-"
  ...
  File.write(ARGV.fetch(6), JSON.pretty_generate(record) + "\n", mode: "wx")
' "$window_before" "$geometry_path" "$transformation_path" "$semantic_path" "$native_capture_path" "$first_rejection_argument" "$frame_evidence"

...
manifest_file="ios/AppStore/screenshot-manifest-v1.1-build15.template.json"
...
if [ "$actual_bundle_identifier" != "$bundle_identifier" ] || \
   [ "$marketing_version" != "1.1" ] || [ "$build_number" != "15" ] || \
   [ ! -f "$executable_path" ] || [ -L "$executable_path" ]; then
  echo "error: built app identity differs from build 15 capture contract" >&2
  exit 65
fi
...
/usr/bin/ruby -rjson -e '
  ...
  record = {
    ...
    "planManifest" => {
      "file" => "ios/AppStore/screenshot-manifest-v1.1-build15.template.json",
